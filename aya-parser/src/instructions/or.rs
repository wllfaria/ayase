...
pub fn parse_or(input: &str) -> IResult<&str, Instruction> {
    alt((
        |input| lit_reg(input, "or", Instruction::OrLitReg),
        |input| reg_reg(input, "or", Instruction::OrRegReg),
    ))(input)
}
