...
pub fn parse_lsh(input: &str) -> IResult<&str, Instruction> {
    alt((
        |input| lit_reg(input, "lsh", Instruction::LshLitReg),
        |input| reg_reg(input, "lsh", Instruction::LshRegReg),
    ))(input)
}
